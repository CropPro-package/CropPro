...
if(!is.null(sample)){
text(df.tern$rachis[sample], df.tern$grain[sample]-0.025, labels=label, xpd=NA, cex=0.9)
}
legend.table<- data_tri[!duplicated(data_tri$Group),]

legend("bottom", c(paste(legend.table$Group), "Samples"), col=c((paste(legend.table$colour)),col), pch=c((as.numeric(as.character(legend.table$pch))),pch), pt.bg=c((paste(legend.table$bg)),bg), pt.cex=1, cex=0.64,xpd=TRUE, ncol=2, inset=c(-0.03,-0.03))
}
